...
public class Ambiente extends Artifact {
    ...
    void mover(String ag, String sentido) {
        ...
        if (sentido.equals("right") && pos < salas.length - 1 && !estaOcupada(pos + 1, idx)) {
            pos++;
        } else if (sentido.equals("left") && pos > 0 && !estaOcupada(pos - 1, idx)) {
            pos--;
        }
        posicoes[idx] = pos;
    ...
    private boolean estaOcupada(int pos, int idxAgente) {
        int outroIdx = idxAgente == 0 ? 1 : 0;
        return pos == posicoes[outroIdx];
    }
}